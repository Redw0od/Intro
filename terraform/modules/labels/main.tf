locals {
  ...
  generated_tags = "${zipmap(
    compact(list("Name", local.namespace != "" ? "Namespace" : "", local.environment != "" ? "Environment" : "", local.stage != "" ? "Stage" : "", "CreatedBy")),
    compact(list(local.id, local.namespace, local.environment, local.stage, "terraform"))
    )}"
