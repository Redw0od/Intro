...
class MySQL
{
    ...
    public function db_common( $value = '' )
    {
        if( is_array( $value ) )
        {
            foreach( $value as $v )
            {
                if( preg_match( '/AES_DECRYPT/i', $v ) || preg_match( '/AES_ENCRYPT/i', $v )  )
                {
                    return true;
                }
                else
        ...
        {
            if( preg_match( '/AES_DECRYPT/i', $value ) || preg_match( '/AES_ENCRYPT/i', $value ) )
            {
                return true;
            }
        }
    ...
    */
    ...
    public function disconnect()
    {
		...
		public function AddItem(){
				...
				$fullLine = mysqli_real_escape_string($this->connection, $this->$fullLine);
				echo "\r\n Printing escape result \r\n";
				var_dump($fullLine);
				echo "\r\n End \r\n";
			/*	$this->type = $this->connection->real_escape_string($this->$type);
				$this->replyCode = $this->connection->real_escape_string($this->$replyCode);
				$this->command = $this->connection->real_escape_string($this->$command);
				$this->commandArgs = $this->connection->real_escape_string($this->$commandArgs);
				$this->ident = $this->connection->real_escape_string($this->$ident);
				$this->sender = $this->connection->real_escape_string($this->$sender);
				$this->receiver = $this->connection->real_escape_string($this->$receiver);
				$this->origin = $this->connection->real_escape_string($this->$origin);
				$this->authLevel = $this->connection->real_escape_string($this->$authLevel); */
				
				$Query = "INSERT INTO `opera`.`Chat` ( `timestamp`, `fullLine`, `type`, `replyCode`, `command`, `commandArgs`, `ident`, `sender`, `receiver`, `origin`, `authLevel`) VALUES( DEFAULT, '".$fullLine."', '".$this->type."', '".$this->replyCode."', '".$this->command."', '".$this->commandArgs."', '".$this->ident."', '".$this->sender."', '".$this->receiver."', '".$this->origin."', '".$this->authLevel."' )";
				//$Query = "SELECT * FROM `opera`.`Chat`";
